...
const subtotal = computed(() =>
	invoice.value.items?.reduce((acc, item) => acc + item.subtotal, 0) ?? 0,
)
const taxAmount = computed(() => subtotal.value * ((invoice.value.tax ?? 0) / PERCENT))
const discountAmount = computed(() => subtotal.value * ((invoice.value.discount ?? 0) / PERCENT))
...
<template lang="pug">
section#body-outlet(v-if="loaded")
	div(class="flex")
		div(:class="['bg-neutral-200 h-screen p-6 lg:w-2/3 overflow-auto', {'!w-full': !showSettings}]")
			NuxtLink(v-if="!showSettings" to="/dashboard" class="absolute top-5 left-4 bg-neutral-800 h-8 px-3 text-sm text-white hover:bg-neutral-700 inline-flex items-center") Dashboard
			...
			.buttons(class="space-x-4 pb-6 flex justify-center")
				...
				button(v-else class="bg-neutral-800 h-9 text-sm px-4 text-white hover:bg-neutral-700")
					svg-spinner(class="mx-auto")
				button(@click="navigateTo('/new')" class="bg-white h-9 text-sm px-4 text-black hover:bg-neutral-100") Edit
			templates-default(v-if="format.template === 'Default'" ref="invoiceRef" :invoice="invoice" :format="format" :logo="logo" :subtotal="subtotal" :taxAmount="taxAmount" :discountAmount="discountAmount" :grandTotal="grandTotal")
			templates-classic(v-else-if="format.template === 'Classic'" ref="invoiceRef" :invoice="invoice" :format="format" :logo="logo" :subtotal="subtotal" :taxAmount="taxAmount" :discountAmount="discountAmount" :grandTotal="grandTotal")
			templates-bubblegum(v-else-if="format.template === 'Bubblegum'" ref="invoiceRef" :invoice="invoice" :format="format" :logo="logo" :subtotal="subtotal" :taxAmount="taxAmount" :discountAmount="discountAmount" :grandTotal="grandTotal")
			...
			.settings-sections(class="space-y-12 *:border-b *:border-zinc-300 *:pb-8 last:border-b-0")
				...
				div
					...
					div(class="text-sm tracking-tight space-y-4")
						...
						p If you have an idea that would help you and other users in creating your invoice more easily send me an email about it.
